...
async function performAccountOperation(account) {
    ...
    try {
        logger.info("Starting operation for account:", account);
        ...
        await Helper.clear(user.status); // Ensure user.status is defined

        await coreService.getSwapTask();
        await coreService.getExchangeTask();
        await coreService.getDiscordTask();

        // Log the coreService object to inspect its methods and properties
        logger.info("CoreService methods and properties:", Object.keys(coreService));
        ...
        if (coreService.exchangeTask.step.find(step => step.status === '0') !== undefined) {
            await coreService.addStep(coreService.exchangeTask.ID, coreService.exchangeTask.step[0]);
        }
        await coreService.getDiscordTask();
